...
This isn't a discretization artifact I need a finer grid to fix. The continuum 2D Dirac delta potential is a well known pathological case in quantum mechanics: unlike 1D and 3D, it has no natural bound-state energy scale, the problem needs an explicit regularization (a cutoff or a renormalized coupling) to have a well-defined answer at all, and the energy runs logarithmically with that cutoff. Here the grid spacing $dx$ is playing the role of that cutoff, so refining the grid doesn't converge toward a hidden true value, it just changes the effective regularization. I kept `V_DeltaDiscrete_2D` in the code because it's still a legitimate, well-defined discrete Hamiltonian for any fixed $N$, and because running into this by actually checking the numbers, instead of assuming the 1D result would just carry over, is a more honest outcome than pretending it isn't there.
